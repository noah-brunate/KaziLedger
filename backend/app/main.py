...
async def lifespan(application: FastAPI):
    ...
    async with httpx.AsyncClient(timeout=10) as auth_client:
        application.state.supabase_auth_client = auth_client
        yield


app = FastAPI(
    title="KaziLedger API", version="1.0.0", openapi_url="/api/v1/openapi.json", lifespan=lifespan
)
app.add_middleware(
    CORSMiddleware,
...
async def current_user(
    ...
    auth_user = await verify_supabase_access_token(
        ...
        client=request.app.state.supabase_auth_client,
        settings=settings,
    )
    user = await db.scalar(select(User).where(User.auth_user_id == auth_user.id))
...
async def next_expert(
    db: AsyncSession, request: ServiceRequest, excluded: set[uuid.UUID]
) -> uuid.UUID | None:
...
async def assign_request(
    db: AsyncSession, request: ServiceRequest, excluded: set[uuid.UUID] | None = None
) -> None:
    excluded = excluded or set()
    expert_id = await next_expert(db, request, excluded)
    request.assigned_expert_id = expert_id
    request.response_due_at = (
        datetime.now(UTC) + timedelta(hours=settings.expert_response_hours) if expert_id else None
    )
    ...
    db.add(
        RequestAssignmentLog(
            ...
            expert_id=expert_id,
            action="assigned" if expert_id else "experts_exhausted",
        )
    )
...
async def assignment_history(db: AsyncSession, request_id: uuid.UUID) -> set[uuid.UUID]:
    rows = (
        await db.scalars(
            select(RequestAssignmentLog).where(RequestAssignmentLog.request_id == request_id)
        )
    ).all()
...
async def release_escrow(
    ...
    amount_to_expert: Decimal | None = None,
) -> dict:
...
async def list_services(db: AsyncSession = Depends(get_db)) -> list[dict]:
    rows = (
        await db.scalars(
            ...
            .where(ServiceSubcategory.active.is_(True))
...
async def get_request(
    request_id: uuid.UUID, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)
) -> dict:
    request = await get_request_for_user(db, request_id, user)
    logs = (
        await db.scalars(
            select(RequestAssignmentLog)
            .where(RequestAssignmentLog.request_id == request_id)
            .order_by(RequestAssignmentLog.timestamp)
        )
    ).all()
...
async def respond(
    ...
    if payload.action == "more_info":
        ...
        db.add(
            RequestAssignmentLog(
                request_id=request.id, expert_id=user.id, action="more_info", reason=payload.reason
            )
        )
    else:
        ...
        db.add(
            RequestAssignmentLog(
                request_id=request.id, expert_id=user.id, action="reject", reason=payload.reason
            )
        )
...
async def reply_to_info(
    request_id: uuid.UUID,
    payload: ClientInfoReply,
    user: User = Depends(current_user),
    db: AsyncSession = Depends(get_db),
) -> dict:
    require_role(user, "client")
    ...
    db.add(
        RequestAssignmentLog(
            request_id=request.id, expert_id=request.assigned_expert_id, action="client_replied"
        )
    )
    await db.commit()
    return {
...
async def create_quote(
    payload: QuoteCreate, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)
) -> dict:
    require_role(user, "expert")
    ...
    band = await db.scalar(
        select(PriceBand).where(
            PriceBand.subcategory_id == request.subcategory_id,
            PriceBand.client_type == client.client_type,
        )
    )
...
async def list_quotes(
    ...
    if user.role == "expert":
        ...
        query = query.join(ServiceRequest, Quote.request_id == ServiceRequest.id).where(
...
async def start_wallet_topup(
    ...
    return {
        ...
        "tracking_id": topup.pesapal_tracking_id,
    }


@app.post("/api/v1/quotes/{quote_id}/charge")
async def charge_quote_from_wallet(
    quote_id: uuid.UUID, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)
) -> dict:
    require_role(user, "client")
    quote = await db.get(Quote, quote_id)
    if not quote:
    ...
    existing = await db.scalar(
        select(EscrowTransaction).where(
            EscrowTransaction.request_id == request.id,
            EscrowTransaction.status.in_(["pending", "funded", "disputed"]),
        )
    )
    if existing:
    ...
    if wallet.balance < quote.amount:
        raise HTTPException(
            422,
            f"Insufficient wallet balance. Top up at least {quote.amount - wallet.balance} {quote.currency}.",
        )
    ...
    escrow = EscrowTransaction(
        ...
        amount=quote.amount,
        status="funded",
        release_at=datetime.now(UTC) + timedelta(hours=settings.escrow_release_hours),
    )
    db.add(escrow)
    await db.flush()
    db.add(
...
async def pesapal_ipn(
    OrderTrackingId: str, OrderMerchantReference: str, db: AsyncSession = Depends(get_db)
) -> dict:
    topup = await db.scalar(
        select(WalletTopUp)
        .where(WalletTopUp.pesapal_tracking_id == OrderTrackingId)
        .with_for_update()
    )
    if not topup:
    ...
    if details.get("payment_status_description") == "Completed" and topup.status != "completed":
        wallet = await db.scalar(
            select(Wallet).where(Wallet.id == topup.wallet_id).with_for_update()
        )
        if not wallet:
...
async def confirm_completion(
    ...
    escrow = await db.scalar(
        select(EscrowTransaction).where(
            EscrowTransaction.request_id == request.id, EscrowTransaction.status == "funded"
        )
    )
...
async def create_dispute(
    ...
    escrow = await db.scalar(
        select(EscrowTransaction).where(
            EscrowTransaction.request_id == request.id,
            EscrowTransaction.status.in_(["funded", "pending"]),
        )
    )
    if not escrow or request.status not in {
...
async def resolve_dispute(
    ...
    escrow = await db.scalar(
        ...
        .where(EscrowTransaction.request_id == dispute.request_id)
        .with_for_update()
    )
...
async def wallet(user: User = Depends(current_user), db: AsyncSession = Depends(get_db)) -> dict:
    ...
    transactions = (
        await db.scalars(
            ...
            .where(WalletTransaction.wallet_id == row.id)
    ...
    topups = (
        await db.scalars(
            select(WalletTopUp)
            .where(WalletTopUp.wallet_id == row.id)
...
async def pending_withdrawals(
    ...
    rows = (
        await db.scalars(
            select(WalletTransaction)
            .where(WalletTransaction.type == "withdrawal", WalletTransaction.status == "pending")
...
async def apply_as_expert(
    ...
    if not await db.get(ServiceSubcategory, payload.subcategory_id):
        raise HTTPException(404, "Service not found")
    existing = await db.scalar(
        select(ExpertSubcategory).where(
            ExpertSubcategory.expert_id == user.id,
            ExpertSubcategory.subcategory_id == payload.subcategory_id,
            ExpertSubcategory.status.in_(["pending", "approved"]),
        )
    )
    if existing:
        raise HTTPException(409, "An active application already exists for this service")
    application = ExpertSubcategory(
        expert_id=user.id,
        subcategory_id=payload.subcategory_id,
        status="pending",
...
async def review_application(
    ...
    return {
        "id": application.id,
        "status": application.status,
        "reason": application.interview_outcome,
    }
...
async def admin_services(
    ...
    rows = (
        await db.scalars(
            select(ServiceSubcategory).order_by(
                ServiceSubcategory.category, ServiceSubcategory.name
            )
        )
    ).all()
...
async def create_service(
    payload: ServiceCreate, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)
) -> dict:
    require_role(user, "admin")
    existing = await db.scalar(
        select(ServiceSubcategory).where(
            func.lower(ServiceSubcategory.name) == payload.name.lower()
        )
    )
    if existing:
...
async def update_service(
    ...
    if "name" in changes:
        duplicate = await db.scalar(
            select(ServiceSubcategory).where(
                func.lower(ServiceSubcategory.name) == changes["name"].lower(),
                ServiceSubcategory.id != service_id,
            )
        )
        if duplicate:
            raise HTTPException(409, "A service with this name already exists")
...
async def create_price_band(
    payload: PriceBandCreate, user: User = Depends(current_user), db: AsyncSession = Depends(get_db)
) -> dict:
    require_role(user, "admin")
    ...
    return {
        "id": band.id,
        "subcategory_id": band.subcategory_id,
        "client_type": band.client_type,
...
async def process_jobs(
        ...
        if request.assigned_expert_id:
            db.add(
                RequestAssignmentLog(
                    ...
                    action="timeout",
                    reason="Expert response window expired",
                )
            )
            request.rejection_count += 1
            if request.rejection_count >= 3:
    ...
    expired_quotes = (
        await db.scalars(select(Quote).where(Quote.status == "pending", Quote.expires_at < now))
    ).all()
    ...
    escrows = (
        await db.scalars(
            select(EscrowTransaction).where(
                ...
                EscrowTransaction.release_at.is_not(None),
    ...
    return {
        ...
        "expired_quotes": len(expired_quotes),
...
async def metrics(user: User = Depends(current_user), db: AsyncSession = Depends(get_db)) -> dict:
    ...
    open_escrow = await db.scalar(
        select(func.coalesce(func.sum(EscrowTransaction.amount), 0)).where(
            EscrowTransaction.status.in_(["funded", "disputed"])
        )
    )
    pending_applications = await db.scalar(
        select(func.count(ExpertSubcategory.id)).where(ExpertSubcategory.status == "pending")
    )
    ...
    return {
        ...
        "settings": {
            "expert_response_hours": settings.expert_response_hours,
...
async def serve_client(full_path: str) -> FileResponse:
    ...
    candidates = (
        ...
        if relative_path
        else [client_static_dir / "index.html"]
    )
